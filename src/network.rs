...
fn scan_network(target_ip: &str) -> Vec<(Ipv4Addr, MacAddr)> {
    ...
    let intefaces = interfaces.into_iter().find(|iface| iface.is_up() && !iface.is_loopback() && iface.ips.iter().any(|ip| ip.ip().is_ipv4())).expect("No suitable ntwrk interface found.");
    ...
    let arp_request = arp::ArpPacket::new(
        ...
        .set_payload(&arp::ArpPacket::new_arp_request(source_mac, source_ip, target_ip).unwrap())
        .build().unwrap()
    );
    ...
    for packet in rx.iter() {
        if let Some(arp_packet) = arp::ArpPacket::new(packet.packet()) {
            if arp_packet.get_operatioon() == arp::ArpOperation::Reply {
                clients.push(arp_packet.get_sender_proto_addr(), arp_packet.get_sender_hw_addr());
            }
        }
    }
